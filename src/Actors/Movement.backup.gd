...
func get_effective_gravity() -> float:
	if Engine.time_scale < 1.0:
		return default_gravity / Engine.time_scale
	return default_gravity
	
func get_effective_max_fall_velocity() -> float:
	var max_fall_velocity = character.maximum_fall_velocity
	if Engine.time_scale < 1.0:
		return max_fall_velocity / Engine.time_scale
	return max_fall_velocity

func process_gravity(delta: float, gravity: float = default_gravity) -> void :
	var _gravity = get_effective_gravity()
	var max_velocity = get_effective_max_fall_velocity()
	character.add_vertical_speed(_gravity * delta)
	if character.get_vertical_speed() > max_velocity:
		character.set_vertical_speed(max_velocity)
...
func set_movement_no_direction(_horizontal_velocity: float, _delta: float = 0.016) -> void :
	character.set_horizontal_speed(_horizontal_velocity * get_pressed_direction())
