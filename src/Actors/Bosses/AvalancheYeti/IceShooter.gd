extends Node2D
class_name IceShooter

const sequence: Array = [0, 1, - 1, 0.5, - 0.5, 0, 0.25, - 0.25, 1, - 1, - 0.75, 0.75]

export (PackedScene) var shards
export  var active: bool = false
export  var debug_logs: bool = false
export  var interval_between_shots: float = 2.5
export  var shot_creation_height: float = - 180

onready var actual_snow: Particles2D = $actual_snow
onready var character: = get_parent().get_parent()

var created_shards: Array = []
var timer: float = 0.0
var shot_position_y: float = 0.0
...
func fire(projectile, _shot_position, _dir: = 0, velocity_override: = Vector2(0, 0)):
