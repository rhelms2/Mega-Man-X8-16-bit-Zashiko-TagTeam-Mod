...
func correct_cannon_rotation() -> void :
	...
	if is_instance_valid(target):
		if target.current_health > 0:
			target_dir = Tools.get_angle_between(target, self) * character.direction.x
	else:
		target_dir = Vector2(1 * character.direction.x, 0)
...
func _Update(_delta: float) -> void :
	...
	if attack_stage == 0:
		...
		rotate.play()
		tween = create_tween()
		tween.tween_property(cannon, "rotation", target_dir.angle() * character.direction.x, 0.65).set_trans(Tween.TRANS_BACK)
		next_attack_stage()
	...
	if attack_stage == 2:
		...
		shot.set_horizontal_speed(200 * target_dir.x * character.direction.x)
		shot.set_vertical_speed(200 * target_dir.y * character.direction.x)
		timer = 0
		next_attack_stage()
