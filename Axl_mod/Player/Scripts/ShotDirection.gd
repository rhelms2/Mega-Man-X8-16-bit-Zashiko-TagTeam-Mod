...
onready var offset_manager: Node2D = character.get_node("offset manager")
onready var x_offset: int = offset_manager.x_offset
onready var y_offset: int = offset_manager.y_offset
onready var extra_x_offset: int = offset_manager.extra_x_offset
onready var extra_y_offset: int = offset_manager.extra_y_offset
onready var weapon_x_offset: int = 0
onready var weapon_y_offset: int = 0

var current_frame: int = 2
var exclusive_ground_animations: Array = [
	"walk_start", 
	"walk", 
	"idle", 
	"weak", 
...
var exclusive_air_animations: Array = [
	"jump", 
	"fall", 
	"walljump", 
	"hover", 
]
var exclusive_wall_animations: Array = [
	"slide", 
]


func check_for_allowed_animations() -> bool:
...
func set_frame_to_direction() -> void :
	var analog_direction = character.get_left_analog_direction()
	
	
	var key_left = character.get_action_pressed("move_left") and analog_direction.length() < 0.32
	var key_right = character.get_action_pressed("move_right") and analog_direction.length() < 0.32
	var key_up = character.get_action_pressed("move_up") and analog_direction.length() < 0.32
	var key_down = character.get_action_pressed("move_down") and analog_direction.length() < 0.32
	var diagonal_tolerance = 0.5
	var move_left = key_left or (analog_direction.x < - diagonal_tolerance)
	var move_right = key_right or (analog_direction.x > diagonal_tolerance)
	var move_up = key_up or (analog_direction.y < - diagonal_tolerance)
	var move_down = key_down or (analog_direction.y > diagonal_tolerance)
...
func projectile_speed_control(_projectile, _speed, horizontal_dir, vertical_dir) -> void :
	var _animation = character.get_animation()
	offset_manager.get_offset()
	x_offset = offset_manager.x_offset
	y_offset = offset_manager.y_offset
	extra_x_offset = offset_manager.extra_x_offset
	extra_y_offset = offset_manager.extra_y_offset
	
	if shot_node.current_weapon != null:
		weapon_offsets()
...
func weapon_offsets() -> void :
	var current_weapon = shot_node.current_weapon
	var temp_x_offset = 0
	var temp_y_offset = 0
