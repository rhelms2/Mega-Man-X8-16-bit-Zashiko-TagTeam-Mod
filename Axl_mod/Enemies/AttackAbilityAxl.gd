class_name AttackAbilityAxl extends EnemyAbilityAxl
...
func is_player_nearby_horizontally(distance: = 24.0) -> bool:
	if GameManager.get_player_position().x > character.global_position.x - distance and GameManager.get_player_position().x < character.global_position.x + distance:
		return true
	else:
		return false
...
func is_player_nearby_vertically(distance: = 24.0) -> bool:
	if GameManager.get_player_position().y > character.global_position.y - distance and GameManager.get_player_position().y < character.global_position.y + distance:
		return true
	else:
		return false
