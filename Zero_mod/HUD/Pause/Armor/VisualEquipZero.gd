...
onready var black_part = part_options[1]
...
func show_parts() -> void :
	for part in part_options:
		part.visible = "black_zero_armor" in GameManager.collectibles
		if part.visible:
			part.modulate = Color.dimgray
			if "black" in part.name and CharacterManager.black_zero_armor or not "black" in part.name and not CharacterManager.black_zero_armor:
				part.modulate = Color.white
				equip(part)
	default_part.visible = true

func black_body_unlocked():
	return "black_zero_armor" in GameManager.collectibles
...
func update_armor(is_black_body: bool) -> void :
	CharacterManager.black_zero_armor = is_black_body
	var texture_path = "res://Zero_mod/HUD/Char_Selection_Zero.png"
	if is_black_body:
		texture_path = "res://Zero_mod/HUD/Char_Selection_Zero_Black.png"
	armor.texture = load(texture_path)
	flash(armor)
