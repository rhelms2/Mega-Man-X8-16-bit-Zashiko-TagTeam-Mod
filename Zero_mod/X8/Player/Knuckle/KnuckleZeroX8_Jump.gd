...
onready var saber_sound2 = get_node("saber2")
...
func hitbox_and_position():
	...
	if landed:
		...
		if animatedSprite.frame >= 1 and animatedSprite.frame < 3:
			hitbox_upleft = Vector2( - 3, - 29)
			hitbox_downright = Vector2(35, 1)
			spawn_hitbox(hitbox_upleft, hitbox_downright)
...
func is_in_buffer_window() -> bool:
	if animatedSprite.animation == "saber_jump_1" and animatedSprite.frame >= 5 - buffer_window_threshold:
		return true
	if animatedSprite.animation == "saber_jump_2" and animatedSprite.frame >= 5 - buffer_window_threshold:
		return true
	return false

func should_add_saber_combo():
	if animatedSprite.animation == "saber_jump_1":
		return animatedSprite.frame >= 5
	if animatedSprite.animation == "saber_jump_2":
		return animatedSprite.frame >= 5
	return false
...
func set_saber_animations():
	if not character.is_on_floor():
		if punches == 0:
			animatedSprite.animation = "saber_jump_1"
			punches += 1
			saber_sound.play()
		elif punches == 1:
			animatedSprite.animation = "saber_jump_2"
			saber_sound2.play()
			punches = 0
...
func _Update(delta: float) -> void :
	...
	if character.dashfall:
		set_movement_and_direction(dashjump_speed, delta)
	else:
		set_movement_and_direction(horizontal_velocity, delta)
