...
const buttons: Array = ["move_left", "move_right", "move_up", "move_down", "alt_fire"]


static func filter_for_type(type, children) -> Array:
	var particle_list = []
...
static func filter_particles(children) -> Array:
	var particle_list = []
	for child in children:
...
static func filter_for_name(text: String, children) -> Array:
	var valid_children = []
	for child in children:
...
static func toggle_emit(object, state: bool) -> void :
	if object is Array:
...
static func flip(object, d: int) -> void :
	if object is Array:
		for particle in object:
			particle.scale.x = d
	else:
...
static func timer_array(target, delay, method_name, params) -> void :
	...
	timer.connect("timeout", target, method_name, [params])
	timer.start()

static func timer(wait_time: float, method: String, parent: Node, method_owner = null, ignore_pause: bool = false) -> void :
	var timer = Timer.new()
